...
HyperSpec_DerivGeneratorRanger<-function(filename,out_file,Classif_Model){
  ...
  if(grepl(".csv",filename) == TRUE){
    ...
    write.csv(Spectral_lib,paste(out_file,"_SpecLib_Derivs",".csv", sep=""),row.names = F)
    ...
    writeRaster(round(Predicted_Layer),filename = paste0(SubFolder,"/",basename(filename),"_PredLayer.tif")
                ,overwrite = T)
